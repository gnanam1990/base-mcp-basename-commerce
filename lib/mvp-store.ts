...
const metricKinds = ["count", "activity", "revenue"] as MetricKind[];
...
const seedItems: MvpItem[] = seedRecords.map((record, index) => ({
  ...
  descriptor: record[1],
  detail: record[2],
  priceUsdc: Number((1.5 + index * 0.01).toFixed(3)),
  usageCount: extractNumber(record[2], 12 + index * 7),
  paidRuns: extractNumber(record[1], 4 + index * 3),
  revenueUsdc: Number(((1.5 + index * 0.01) * (4 + index * 3)).toFixed(3)),
  status: "Active",
  payload: {
    label: record[0],
    descriptor: record[1],
...
function slugify(value: string) {
  const base = value
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  return base || `basename-commerce-item-${Date.now()}`;
}
...
export function recordPaidRun(
  ...
  const receipt: MvpReceipt = {
    id: `basename-commerce-${itemSlug}-${Date.now()}`,
    itemSlug,
    itemName: item.name,
